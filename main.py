...
async def developer(desarrollador: str) -> DeveloperInfo:
    ...
    listaaños = sorted(df_EP1.AÑO.unique(), reverse=True)
    ...
    if desarrollador in listadeveloper: 
        for años in listaaños:
            df_desarrollador = df_EP1[(df_EP1.DEVELOPER == desarrollador) & (df_EP1.AÑO == años)]
            cantidad_total_items = len(df_desarrollador)
            cantidad_items_free = len(df_desarrollador[df_desarrollador.PRECIO == 0])
...
async def userdata(usuario: str) -> Union[UserDataResponse, dict]:
    ...
    if usuario in df_EP2.ID_USUARIO.unique():
        # Filtramos el DataFrame por el usuario especificado
        df_usuario = df_EP2[df_EP2.ID_USUARIO == usuario]

        # Calculamos la cantidad de dinero gastado
        dinero_gastado = round(df_usuario.PRECIO.sum(), 2)

        # Calculamos el porcentaje de recomendación
        porcentaje_recomendacion = int((df_usuario.RECOMIENDA.mean()) * 100)
...
async def UserForGenre(genero: str) -> Dict[str, Union[str, List[Dict[str, Union[int, str]]]]]:
    ...
    df_genero = df_EP3[df_EP3['GENERO'].str.lower() == genero.upper()]
    ...
    usuario_mas_horas = df_genero.groupby('ID_USUARIO')['HORAS_DE_JUEGO'].sum().idxmax()

    # Creamos una lista de la acumulación de horas jugadas por año de lanzamiento, ordenada de forma descendente por año
    horas_por_año = df_genero.groupby('AÑO_LANZAMIENTO')['HORAS_DE_JUEGO'].sum().reset_index()
    horas_por_año = horas_por_año.rename(columns={'AÑO_LANZAMIENTO': 'Año', 'HORAS_DE_JUEGO': 'Horas'})
...
async def best_developer_year(year):
    ...
    recom = df_EP4[(df_EP4["RECOMIENDA"] == True) & (df_EP4["SENTIMIENTO"] > 0) & (df_EP4["AÑO"] == year)]
...
async def DeveloperReviewsAnalysis(desarrolladora: str) -> Union[Dict[str, Dict[str, int]], Dict[str, str]]:
    ...
    desarrolladora = desarrolladora.upper()
    ...
    df_EP5['DEVELOPER'] = df_EP5['DEVELOPER'].str.upper()
    ...
    df_filt_developer = df_EP5[(df_EP5['DEVELOPER'] == desarrolladora) & (df_EP5['SENTIMIENTO'] != 1)]
    ...
    if not df_filt_developer.empty:
        ...
        sentiment_counts = df_filt_developer['SENTIMIENTO'].replace({0: 'Negative', 1: 'Positive', 2: 'Positive'}).value_counts()
...
async def recomendacion_juego(id: str):
    ...
    game = modelo_games[modelo_games['ID_GAME'] == str(id)]
